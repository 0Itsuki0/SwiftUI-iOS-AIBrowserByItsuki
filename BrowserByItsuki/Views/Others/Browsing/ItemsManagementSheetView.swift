...
struct ItemsManagementSheetView: View {
    ...
    @State var selectedItemType: SavedItemType = .readingList
    ...
    private func bookmarksView(_ bookmarks: [Bookmark]) -> some View {
        if bookmarks.isEmpty {
            ...
            Section {
                ...
                HStack {
                    Text(SavedItemType.bookmark.title)
                    Spacer()

                    Button(action: {
                        self.browserManager.removeBookmarks(self.browserManager.bookmarks)
                    }, label: {
                        Text("Remove All")
                    ...
                    .disabled(self.browserManager.bookmarks.isEmpty)
                }
            }
        }
    }
    ...
    private func readingListView(_ readingList: [ReadingListItem]) -> some View {
        if readingList.isEmpty {
            ...
            Section {
                ...
                HStack {
                    Text(SavedItemType.readingList.title)
                    Spacer()

                    Button(action: {
                        self.browserManager.removeReadingListItems(self.browserManager.readingItems)
                    }, label: {
                        Text("Remove All")
                            .foregroundStyle(.gray)
                            .fontWeight(.medium)
                    ...
                    .buttonStyle(.borderless)
                    .disabled(self.browserManager.readingItems.isEmpty)
                }
                ...
                if let date, !histories.isEmpty {
                    Section {
                        ...
                        HStack {
                            ...
                            if dateComponent == keys.first {
                                Button(action: {
                                    self.browserManager.removeHistories(self.browserManager.histories)
                                }, label: {
                                    Text("Clear All")
                                        .foregroundStyle(.gray)
                                        .fontWeight(.medium)
                                ...
                                .buttonStyle(.borderless)
                                .disabled(self.browserManager.histories.isEmpty)
                            }
                        }
